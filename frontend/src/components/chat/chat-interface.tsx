...
export function ChatInterface({ documentCount, onDocumentUploaded }: ChatInterfaceProps) {
  ...
  const handleSend = useCallback(async (content: string, model: string = "claude-3-5-sonnet") => {
    ...
    try {
      // 1. Retrieve RAG context & citations from backend pgvector with document scoping
      ...
      try {
        ...
        await streamQuestion(content, targetDocIds || [], undefined, (event: StreamEventPayload) => {
